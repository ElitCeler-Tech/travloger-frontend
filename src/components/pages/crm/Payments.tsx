...
const Payments: React.FC = () => {
  ...
  const fetchPayments = async () => {
    try {
      setLoading(true)
      const data = await fetchApi<{ payments: Payment[] }>('/api/payments')
      setPayments(data.payments || [])
      setError(null)
    } catch (err: any) {
      const message = handleApiError(err, 'Failed to load payments')
      setError(message)
    } finally {
      setLoading(false)
    }
  ...
  const handleDeletePayment = async (paymentId: number): Promise<void> => {
    try {
      ...
      const message = handleApiError(error, 'Failed to delete payment record')
      alert(message)
    } finally {
      setDeleting(false)
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">Track payment methods, transaction details, and financial records</p>
        </div>

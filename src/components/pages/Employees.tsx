...
const Employees: React.FC = () => {
  ...
  const safeString = (value: unknown, fallback: string): string => {
    ...
    // Handle destination objects (or any object with name/destination property)
    if (typeof value === 'object' && value !== null) {
      const obj = value as any
      if (obj.name) return obj.name
      if (obj.destination) return obj.destination
    }
    try {
      return String(value)
    } catch {
  ...
  const handleDeleteEmployee = async () => {
    ...
    const ok = window.confirm(
      `Delete employee "${editingEmployee.name}" (${editingEmployee.email})? This cannot be undone.`
    )
    if (!ok) return
    try {
      ...
      const message = handleApiError(error, 'Failed to delete employee')
      alert(message)
    }
  }
  ...
  const handleDeleteEmployeeFromModal = async (employeeId: number): Promise<void> => {
    try {
      ...
      const message = handleApiError(error, 'Failed to delete employee record')
      alert(message)
    } finally {
      setDeleting(false)
    }
  ...
  const fetchActiveSessions = async () => {
    try {
      ...
      const data = await fetchApi<{ activeEmployeeIds?: number[] }>('/api/employees/active-sessions')
  ...
  const handleSaveEmployee = async () => {
    ...
    try {
      const data = await fetchApi<{ employee?: Employee }>(`/api/employees/${editingEmployee.id}`, {
        method: 'PUT',
      ...
      const message = handleApiError(error, 'Failed to update employee. Please try again.')
  ...
  const fetchEmployees = async (): Promise<void> => {
    try {
      const data = await fetchApi<{ employees?: Employee[] }>('/api/employees')
      if (data.employees) {
        setEmployees(
      ...
      const message = handleApiError(error, 'Employees fetch failed')
      setErrorMessage(message)
    }
  }
  ...
  const handleCreateEmployee = async (): Promise<void> => {
    ...
    try {
      const data = await fetchApi<{ employee?: Employee; emailSent?: boolean }>(`/api/employees`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (data.employee) {
        ...
        setSuccessMessage(
          data.emailSent
            ...
            : 'Employee created successfully! However, there was an issue sending the email. Please share credentials manually.'
        )
      }
    } catch (error) {
      const message = handleApiError(error, 'Failed to create employee')
      setErrorMessage(message)
    }
  }
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                onClick={async () => {
                  try {
                    await fetchApi('/api/employees/cleanup-sessions', { method: 'POST' })
                    fetchActiveSessions()
                  } catch (error) {

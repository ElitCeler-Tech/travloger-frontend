...
export const AboutSection: React.FC<AboutSectionProps> = ({ packageData }) => {
  const summaryText = packageData?.summary_text || packageData?.short_description || '';
  ...
  return (
    <div className="relative py-28 bg-[#2d3032] text-white min-h-[70vh] flex items-center w-full" style={{ fontFamily: "'Poppins', sans-serif" }}>
      <style>{`
      ...
      <div className="absolute inset-0 w-full h-full overflow-hidden">
        <img
          src={summaryImage || "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=1920"}
        ...
        <div className="w-full lg:w-[55%] flex flex-col gap-8 md:gap-12">
          ...
          >
            {summaryText ? (
              ...
              <p>
                Traveloger passionately crafts exceptional travel experiences, serving <strong className="text-[#f27a3a] font-semibold">ten million customers</strong> as India's leading online platform for booking travel experiences in <strong className="text-[#f27a3a] font-semibold">55+ countries.</strong>
              </p>
            )}

            {visionAbout?.description && (
              <p>{visionAbout.description}</p>
            )}
            ...
            {summaryHighlights.length > 0 && (
              <div className="flex flex-wrap gap-3 mt-4">
                {summaryHighlights.map((highlight: string, idx: number) => (
                  ...
                  >
                    <svg className="w-4 h-4 text-[#f27a3a]" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/></svg>
                    {highlight}
                  </span>
                ))}
              </div>
            ...
            >
              <div className="w-10 h-10 rounded-full bg-white/10 border border-white/20 flex items-center justify-center overflow-hidden">
                {packageData?.partner_profile_photo ? (
                  <img src={packageData.partner_profile_photo} alt={partnerName} className="w-full h-full object-cover" />
                ) : (
                  <span className="text-white font-bold text-sm">{partnerName.charAt(0)}</span>
                )}
              </div>
              <div>
